...
const SIM_WIDTH: u32 = 10300;
const SIM_HEIGHT: u32 = 10300;
    ...
    pub fn update(&mut self) {
        for particle in self.state.particles.iter_mut() {
                ...
                && particle.y < (SIM_HEIGHT - 1) as f32
            {
                ...
                let left = self.state.fabric[x - 1][y];
                let right = self.state.fabric[x + 1][y];
                let top = self.state.fabric[x][y - 1];
                let bottom = self.state.fabric[x][y + 1];

                let x_slope = right - left;
                let y_slope = bottom - top;

                let gravity_strength = 0.0004;
                ...
                if let Some(pixels) = &mut self.pixels {
                    ...
                    for sx in 0..VIEW_WIDTH {
                        for sy in 0..VIEW_HEIGHT {
                            //let screen_y = self.offset_y + y as f32;
                            //let screen_x = self.offset_x + x as f32;

                            let world_x = (self.camera_x
                                + (sx as f32 - VIEW_WIDTH as f32 / 2.0) / self.zoom)
                                as i32;
                            let world_y = (self.camera_y
                                + (sy as f32 - VIEW_HEIGHT as f32 / 2.0) / self.zoom)
                                as i32;
                            ...
                            {
                                let depth = self.state.fabric[world_x as usize][world_y as usize];
                                intensity = (depth * 55.0).clamp(0.0, 255.0) as u8;
                            }
